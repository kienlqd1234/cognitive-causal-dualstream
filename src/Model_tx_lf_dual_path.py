...
from MSINModule_dual_path import MSINCell, DualPathwayMSIN, DualPathwayMSINStateTuple
#from MSINModule_MHA import MSINCell, MSIN, MSINStateTuple
import tensorflow.contrib.distributions as ds
from tensorflow.contrib.layers import batch_norm
from ConfigLoader_tx_lf_dual_path import logger, ss_size, vocab_size, config_model, path_parser
...
class Model:
    ...
    def _create_msg_embed_layer(self):
        ...
        def _for_one_trading_day(daily_in, daily_ss_index_vec, daily_mask):
            ...
            # Apply gradient checkpointing to the RNN for memory efficiency
    ...
    def _build_mie(self):
        ...
        with tf.name_scope('mie'):
            ...
            if self.variant_type == 'tech':
                ...
                # Create initial state for DualPathwayMSIN
                initial_state = DualPathwayMSINStateTuple(
                    h=tf.zeros([self.batch_size, self.msin_h_size]),
                    v_causal=tf.zeros([self.batch_size, self.msg_embed_size]),
                    v_responsive=tf.zeros([self.batch_size, self.msg_embed_size])
                )
                ...
                # Set cross-pathway communication parameters
                cell.cross_communication_enabled = self.enable_cross_pathway
                cell.causal_to_responsive_strength = self.c2r_strength
                cell.responsive_to_causal_strength = self.r2c_strength
                ...
                # Store attention components for later use
                ...
                # For backward compatibility with original model architecture
                # Use causal attention as the default P for existing model components
                self.P = self.P_causal
    ...
    def _create_generative_ata(self):
        ...
        with tf.name_scope('ata'):
            with tf.variable_scope('ata'):
                ...
                # Dual-pathway L2 regularization
                lambda_causal = 0.1  # Regularization weight for causal pathway
                lambda_responsive = 0.05  # Lower weight for responsive pathway
                lambda_diff = 0.02  # Weight for pathway differentiation
                ...
                msg_weight = tf.clip_by_value(1/msg_num, 1e-30, 1)
                nonzero = tf.to_float(tf.count_nonzero(self.n_msgs_ph, axis=-1))
                    
                # Causal pathway regularization (encourage sparsity)
                P_causal = tf.clip_by_value(self.P_causal, 1e-30, 1)
                P_causal_reg = tf.reduce_sum(tf.multiply(self.P_causal, tf.multiply(tf.log(P_causal), msg_weight) + 1), axis=[1, 2])
                P_causal_reg = tf.divide(P_causal_reg, nonzero)
                    
                # Responsive pathway regularization (encourage sparsity but less strongly)
                P_responsive = tf.clip_by_value(self.P_responsive, 1e-30, 1)
                P_responsive_reg = tf.reduce_sum(tf.multiply(self.P_responsive, tf.multiply(tf.log(P_responsive), msg_weight) + 1), axis=[1, 2])
                P_responsive_reg = tf.divide(P_responsive_reg, nonzero)
                    
                # Pathway differentiation loss (encourage pathways to focus on different texts)
                P_diff = tf.reduce_sum(tf.multiply(self.P_causal, self.P_responsive), axis=[1, 2])
                P_diff = tf.divide(P_diff, nonzero)
                    
                # Add all regularization components to final loss
                self.loss = (self.loss + 
                ...
                # Add in _create_generative_ata after calculating self.loss
                # Track cross-pathway communication metrics
                with tf.name_scope('cross_communication_metrics'):
                    ...
                    # Calculate information gain from cross-pathway communication
                    # (this is for monitoring - actual communication happens in MSINCell)
                    causal_influence = tf.reduce_mean(self.k_causal)
                    responsive_influence = tf.reduce_mean(self.k_responsive)
                    
                    # Add summaries to track these values during training
                    tf.summary.scalar('pathway_correlation', correlation)
                    tf.summary.scalar('causal_pathway_influence', causal_influence)
                    tf.summary.scalar('responsive_pathway_influence', responsive_influence)
        ...
        def _create_discriminative_ata(self):
            ...
            with tf.name_scope('ata'):
                with tf.variable_scope('ata'):
                    ...
                    P_causal = tf.clip_by_value(self.P_causal, 1e-8, 1)
                    P_causal_obj = tf.reduce_sum(tf.multiply(self.P_causal, tf.log(P_causal)), axis=-1)
                        
                    P_responsive = tf.clip_by_value(self.P_responsive, 1e-8, 1)
                    P_responsive_obj = tf.reduce_sum(tf.multiply(self.P_responsive, tf.log(P_responsive)), axis=-1)
                        
                    # Weight the pathways differently
                    lambda_causal = 0.1
                    lambda_responsive = 0.05
                        
                    self.loss = tf.reduce_mean(-obj, axis=[0, 1])
                    self.loss = (self.loss + 
                                lambda_causal * tf.reduce_mean(-P_causal_obj, axis=[0, 1]) +
                                lambda_responsive * tf.reduce_mean(-P_responsive_obj, axis=[0, 1]))
