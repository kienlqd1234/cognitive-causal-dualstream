...
class MSINCell(object):        
    ...
    def __init__(self, input_size, num_units, v_size, max_n_msgs, forget_bias=1.0):
        ...
        # Add price embedding projection for responsive attention
        with vs.variable_scope('price_embedding'):
            self.W_price_embed = tf.get_variable('W_price_embed', 
                                               shape=[input_size, num_units], 
                                               dtype=tf.float32, 
                                               initializer=xavier_initializer())
            self.b_price_embed = tf.Variable(tf.zeros(num_units), dtype=tf.float32)
        ...
        with vs.variable_scope('cross_pathway_communication'):
            ...
            with vs.variable_scope('cross_projections'):
                ...
                # Add these variables to the init method
                self.causal_proj = tf.get_variable('causal_proj', 
                                           shape=[v_size, v_size], 
                                           dtype=tf.float32, 
                                           initializer=xavier_initializer())
                
                self.responsive_proj = tf.get_variable('responsive_proj', 
                                               shape=[v_size, v_size], 
                                               dtype=tf.float32, 
                                               initializer=xavier_initializer())
                
                self.v_to_text_causal = tf.get_variable('v_to_text_causal', 
                                               shape=[v_size, v_size], 
                                               dtype=tf.float32, 
                                               initializer=xavier_initializer())
                
                self.v_to_text_responsive = tf.get_variable('v_to_text_responsive', 
                                                   shape=[v_size, v_size], 
                                                   dtype=tf.float32, 
                                                   initializer=xavier_initializer())
                
                self.gate_W = tf.get_variable('gate_W', 
                                       shape=[num_units + 2*v_size, 2],
                                       dtype=tf.float32, 
                                       initializer=xavier_initializer())
    ...
    def _cross_pathway_communication(self, h, text_causal, text_responsive, v_causal, v_responsive):
        ...
        with tf.name_scope('cross_pathway'):
            # Use pre-defined variables instead of creating new ones
            
            # Project memory states
            v_causal_proj = tf.matmul(v_causal, self.causal_proj)
            v_responsive_proj = tf.matmul(v_responsive, self.responsive_proj)
            ...
            base_gates = tf.sigmoid(tf.matmul(gate_features, self.gate_W) + self.gate_b)
            ...
            # Cross-enhancement
            text_causal_enhanced = text_causal + r2c_gate * tf.matmul(text_responsive, self.responsive_proj)
            text_responsive_enhanced = text_responsive + c2r_gate * tf.matmul(text_causal, self.causal_proj)
            
            return text_causal_enhanced, text_responsive_enhanced

    def call(self, X, S, state, prev_hidden_state=None):
        # Try to directly unpack the state tuple
        h, v_causal, v_responsive = state
        
        # Create different query vectors for each pathway
        # --------------------------------------------------
        
        # For causal pathway: use previous hidden state (or current if not available)
        # This focuses on text that predicts future price movements
        causal_query = prev_hidden_state if prev_hidden_state is not None else h
        
        # For responsive pathway: use price embedding as query
        # This focuses on text that responds to price movements
        price_embedding = tf.tanh(tf.matmul(X, self.W_price_embed) + self.b_price_embed)
        responsive_query = price_embedding
        
        # 1. Causal pathway attention (text → price) - TSU
        with tf.name_scope('causal_tsu'):
            # Use causal_query (previous hidden state) to attend
            a_causal = tf.tanh(tf.matmul(S, tf.reshape(tf.tile(self.W_sa_causal, [tf.shape(S)[0],1]), 
                             ...
                             tf.reshape(tf.tile((tf.matmul(causal_query, self.W_ha_causal) + self.b_a_causal), [tf.shape(S)[1], 1]), 
        ...
        with tf.name_scope('responsive_tsu'):
            # Use responsive_query (price embedding) to attend
            a_responsive = tf.tanh(tf.matmul(S, tf.reshape(tf.tile(self.W_sa_responsive, [tf.shape(S)[0],1]), 
                                 ...
                                 tf.reshape(tf.tile((tf.matmul(responsive_query, self.W_ha_responsive) + self.b_a_responsive), [tf.shape(S)[1], 1]), 
...
class MSIN(object):
    ...
    def _dynamic_msin_loop(self,
                      ...
                      dtype=None):
        """
        Modified version of MSIN._dynamic_msin_loop that handles directional queries
        for causal and responsive pathways.
        """
        state = initial_state
        ...
        # Create a fixed zero tensor for previous hidden state
        # instead of using a TensorArray
        prev_h_zeros = tf.zeros([batch_size, cell._num_units], dtype=self._infer_state_dtype(dtype, state))

        def _time_step(time, output_ta_t, P_ta_t, state, prev_h):
            if self.in_graph_mode:
            ...
            # Pass the previous hidden state to the cell
            call_cell = lambda: cell.call(input_t, s_input_t, state, prev_h)
            ...
            # Extract the current hidden state to use as prev_h in next step
            current_h = new_state[0]
            
            # Apply sequence_length mask if needed
            if sequence_length is not None:
                mask = tf.less(time, sequence_length)
                mask = tf.reshape(mask, [-1, 1])  # [batch_size, 1]
                mask = tf.tile(mask, [1, cell._num_units])  # [batch_size, hidden_size]
                current_h = tf.where(mask, current_h, prev_h_zeros)
            ...
            return (time + 1, output_ta_t, P_ta_t, new_state, current_h)

        # Initialize with zeros for prev_h
        _, output_final_ta, P_final_ta, final_state, _ = control_flow_ops.while_loop(
            cond=lambda time, *_: time < time_steps,
            body=_time_step,
            loop_vars=(time, output_ta, P_ta, state, prev_h_zeros),
    ...
    def dynamic_msin(self,
        ...
        # Create a TensorArray to store hidden states for temporal dependencies
        #hidden_ta = _create_ta(
        #    "hidden_states",
        #    element_shape=(tensor_shape.TensorShape([const_batch_size]).concatenate(
        #                  tensor_shape.TensorShape([cell._num_units]))),
        #    dtype=self._infer_state_dtype(dtype, state)
        #)
...
class DualPathwayMSIN(MSIN):
    ...
    def dynamic_dual_msin(self, cell, inputs, s_inputs, sequence_length=None, 
        ...
        with vs.variable_scope(scope or "dual_msin") as varscope:
            if varscope.caching_device is None:
                varscope.set_caching_device(lambda op: op.device)
        ...
        # Use our custom _dynamic_msin_loop to properly handle hidden states
        (outputs, P, final_state) = self._dynamic_msin_loop(
            cell,
            inputs,
            s_inputs,
        ...
        # Unpack the combined attention values into a structured dictionary
        # IMPORTANT: Use the cell's max_n_msgs directly to ensure consistency
        P_dict = self.unpack_attention_values(P, cell.max_n_msgs)
