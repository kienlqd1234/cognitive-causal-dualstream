...
class MSINCell(object):        
    ...
    def call(self, X, S, state):
        ...
        P = tf.nn.softmax(tf.transpose(tf.matmul(a, tf.reshape(tf.tile(self.v_a, [tf.shape(S)[0]]), [tf.shape(S)[0] , self._num_units, 1])), perm = [0, 2, 1])) # Normalize attention weights
        
        new_P = tf.squeeze(P, 1)

        # Combine textual and numerical information
        text = tf.squeeze(tf.matmul(P, S), 1) # Weighted message sum
        # Gate mechanisms
        F = tf.sigmoid(tf.matmul(text, self.W_f) + tf.matmul(h, self.W_hf)  + self.b_f) # Forget gate
        O = tf.sigmoid(tf.matmul(text, self.W_o) + tf.matmul(h, self.W_ho)  + self.b_o) # Output gate
