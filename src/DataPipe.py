...
class DataPipe:
    ...
    def index_token(self, token_list, key='id', type='word'):
        ...
        if key == 'id':
            ...
            for id in range(len(token_list_cp)):  # Ensure id is within bounds
                try:
                    token = token_list_cp[id]
                    indexed_token_dict[token] = id
                except (IndexError, KeyError) as e:
                    print(f"Error accessing token_list_cp[{id}]: {e}")
                continue
            #for id in range(len(token_list_cp)):
